...
use thesis_full_sweep_methods::{THESIS_FULL_SWEEP_GUEST_ELF, THESIS_FULL_SWEEP_GUEST_ID};

#[derive(Parser, Debug)]
#[clap(author, version, about = "E5 full_sweep thesis host (isolated from minimal_add)")]
struct Args {
    #[clap(long)]
    trace: bool,
...
fn main() {
    ...
    let prove_info = match prover.prove_with_opts(executor_env, THESIS_FULL_SWEEP_GUEST_ELF, &opts) {
        Ok(prove_info) => {
            println!(
                "<record>{{\
                    \"context\":\"Prover\", \
    ...
    let timer = Instant::now();
    match receipt.verify(THESIS_FULL_SWEEP_GUEST_ID) {
        Ok(_) => {
            println!(
                "<record>{{\
                    \"context\":\"Verifier\", \
