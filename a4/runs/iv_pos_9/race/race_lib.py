#!/usr/bin/env python3
"""Seam-B bug-race analysis library (IV_POS_9_A3_ANALYSIS_SPEC).

Computes the race metrics (ProG_Report_5 §3.7 + race-spec §3) live from the campaign
run-DBs and returns matplotlib Figures for the notebook. Reuses the race oracle
(structural F8 find = decode-divergent INSTR_TYPE_MOD accept; control-confirm is the
G-REPRO spot-check, not needed for counts) and the markers module — NO hardcoded numbers.

Parameterized by RESULTS_DIR; robust to PARTIAL campaigns (reports seeds_done). The same
code renders the smoke (N=120), the live/partial thesis (N=5000), and later the CVE race.

    RESULTS_DIR=a4/runs/iv_pos_9/race/thesis_results python3 a4/runs/iv_pos_9/race/race_lib.py
"""
...
_RID = re.compile(r"a3seamb_(?P<variant>.+?)_seed(?P<seed>\d+)_n(?P<n>\d+)")
