"""
LOAD_VAL_MOD Mutation (Arguzz-Dependent)

This mutation targets the txns[].word field for WRITE transactions in the
PreflightTrace, corresponding to Arguzz's LOAD_VAL_MOD mutation which changes
the loaded value before writing to the destination register.

When Arguzz mutates a load value:
1. The memory is read (ctx.load_memory)
2. The value is extracted/sign-extended based on load type (Lw, Lh, Lb, etc.)
3. Arguzz changes the output value to a MUTATED value before ctx.store_register()
4. The MUTATED value is written to the destination register

A4's LOAD_VAL_MOD directly mutates the WRITE transaction's word field to match
the effect of Arguzz's mutation, testing the circuit's memory consistency checks.

Structurally identical to COMP_OUT_MOD, but specifically for load instructions:
- Lw (load word)
...
Transaction Structure for Load Instructions:
1. Instruction fetch (READ) - addr = PC/4
2. Load rs1 (READ) - base address register
3. Load from memory (READ) - actual memory access
4. Store rd (WRITE) - destination register ← LOAD_VAL_MOD target
"""
...
from a4.core.trace_parser import A4CycleInfo, A4StepTxns, A4Txn
from a4.core.executor import run_a4_inspection_with_step
...
class LoadValModTarget:
    """The target location in A4 for a LOAD_VAL_MOD mutation"""
    ...
    original_value: int     # Original loaded value (from Arguzz fault)
    mutated_value: int      # Mutated loaded value (from Arguzz fault)
...
def find_write_transaction(txns: List[A4Txn], step_txns: A4StepTxns) -> Optional[A4Txn]:
    ...
    For load instructions, there should be exactly one WRITE transaction
    which writes the loaded value to the destination register.
    ...
    if len(write_txns) > 1:
        # For load instructions, there should typically be only one WRITE
...
def find_mutation_target(
    ...
    Args:
        arguzz_fault: The parsed Arguzz LOAD_VAL_MOD fault
...
def create_config(target: LoadValModTarget, output_path: Path) -> Path:
