...
- **V8 has NO arms (lead correct).** "Arguzz's scheduler" = balanced round-robin over the **distinct RISC-V instruction types** in the trace (the `ArguzzScheduler` `_counter` over mnemonics — rare instructions sampled as often as common ones), then a uniform step within that instruction. It does **not** touch the A4 arm universe. So **V8 = Arguzz instruction-balanced site-selection → apply an A4 `INSTR_TYPE_MOD` mutation at that site.** Implementation = a **new driver forked from `v6_uniform_driver.py`** swapping the Arguzz `--inject` for `run_a4_mutation` (~150 LOC; NOT a cli selector).
