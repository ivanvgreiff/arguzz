...
> ⚠️ **ROUND-2 SUPERSEDES the V8 + V0 implementation here.** **V8 is NOT a `RoundRobinArmSelector` over arms** — it is a **new driver forked from `v6_uniform_driver.py`** (Arguzz instruction-balanced `.pick()` → executor→`user_cycle` translation via `step_domain_map.to_user` → `run_a4_mutation` with `kind=INSTR_TYPE_MOD`; 3 DB must-dos: `outcome='applied'`, `kind='INSTR_TYPE_MOD'`, run-dir matches the `_RID` regex). **V0** = `SemanticUniformArmSelector` over `SemanticArmUniverse.available_arms` (the up-to-date 5-field `ArmKey`), wired like the existing `uniform` path but over the semantic universe. **Binary = FIXED + re-run V5 AND Hybrid** (confirmed; 40 jobs; reuse V6). The §Binary *analysis* below is correct; the §A code-shape for V8/V0 is superseded by `00 ROUND-2` + `04`. Also: the `_RID` regex needs generalizing for the `a3seambfix` slug (see `03`).
