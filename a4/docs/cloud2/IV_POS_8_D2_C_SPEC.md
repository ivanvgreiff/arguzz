...
- **Batch-1 real-binary gating for the 7 non-SELECTED V6 kinds** (`POST_EXEC_PC_MOD`, `POST_EXEC_MEM_MOD`, `POST_EXEC_REG_MOD`, `COMP_OUT_MOD`, `LOAD_VAL_MOD`, `STORE_OUT_MOD`, `PRE_EXEC_REG_MOD`) — these ARE in V6-cTS's FULL 11-kind scope (per §1.1.A / §3.4, v0.4), but their dedicated real-binary smoke is *not* a Batch-1 gate; they are exercised at the kind-string level by the Layer-1 mock (Batch 1) and operationally by the Layer-5 V6-uniform driver smoke (Batch 3). Only the 4 SELECTED kinds gate Batch 1's Layer-2 test. *(v0.5 drift fix: pre-v0.4 wording here said these 7 were "Pro deprioritized; out of scope" — that contradicted v0.4's V6-cTS = 11 decision and is corrected.)*
...
- Full pytest sweep at or above the post-D2.B count: **616 passed, 17 skipped** (D2-regression path, `--ignore=a4/standalone/tests/test_run_replicates.py`) at HEAD `dfd0ebe`; **632 passed, 17 skipped** with replicates (649 collected) — no regressions.
...
**Test count expectation (v0.5 refreshed to HEAD `dfd0ebe`):**
- Pre-D2.C baseline at **HEAD `dfd0ebe`** (the D2.B-PS-2 commit): **616 passed, 17 skipped, 0 xfailed** (verified 2026-06-20 via `python -m pytest a4/standalone/tests/ -q --ignore=a4/standalone/tests/test_run_replicates.py`, 154 s; 633 collected). With `test_run_replicates.py` (16 tests, all green): **632 passed, 17 skipped** (649 collected, ~16 min). *(History: v0.3 recorded 609 passed / 19 skipped / 1 xfailed measured the same ignore-replicates way at the earlier commit `e2c2256`; PS-2 at `dfd0ebe` added test edits and converted the lone `CYCLE_DIFF_COUNT_MOD` xfail to a pass — see `IV_POS_8_D2_PLAN.md` §9d / line 763 affected-file slice 168→173. The Δ from `e2c2256`→`dfd0ebe` on the ignore-replicates path is +7 passed / −2 skipped / −1 xfailed = +4 collected.)*
...
## 15. Tracked implementation issues (LIVING ANNEX — updated post-lock)

> **This section is a living issue registry, NOT part of the locked v0.5 design.** It records foreseen/surfaced implementation risks discovered during batch execution. Convention: each issue has a stable ID, the batch that raised it, a **Status** (`OPEN` / `PARTIALLY RESOLVED` / `RESOLVED` / `WON'T-FIX`), and a resolution note. When an issue is resolved, **flip its Status to `RESOLVED` and append the resolving batch/commit** (do not silently delete — keep the audit trail). Batch kickoffs reference these IDs.

| ID | Issue | Raised | Status | Resolution / next step |
|----|-------|--------|--------|------------------------|
| **ISS-1** | **Arguzz `<fault>` tags require `--trace`.** The dev `risc0-host` emits **zero `<fault>` tags under `--inject` alone**; fault parsing needs `--trace --inject`. Batch 1 added `arguzz_invoke.run(..., include_trace: bool = False)`. The bridge/driver must decide whether to pass it. | Batch 1 (Composer + Opus review, 2026-06-20) | **RESOLVED** (bridge half, Batch 2) | **`include_trace=False` shipped in `arguzz_bridge.create_mutation_for_arm`** (Batch 2, task 2.1) with ISS-1 comment at call site. Outcome path is `prover_status`-PRIMARY; production loop does not consume `<fault>` tags. **Residual (OPEN → D2.G):** fault-tag *corroboration* of `soundness_signal` candidates (§6.3) requires a targeted `--trace` rerun on the ~3 % flagged rows — design that into D2.G triage, not the production campaign. |
| **ISS-2** | **V6-cTS FULL arm space likely exceeds the 300 advisory.** Batch-1 paper estimate (task 1.8): **FULL = 437**, **SELECTED = 180** — above §1.2's ~200–350 / ~110–160 bands and §9 Q9's 300 "consider reducing" threshold. | Batch 1 (task 1.8 estimate, 2026-06-20) | **RESOLVED — KEEP 437** (Opus ruling, 2026-06-20) | **Batch 2 task 2.5 authoritative measurement (2026-06-20):** FULL = **437**, SELECTED = **180** via `SemanticArmUniverse.build` on real sha2-host (3962-step `host --trace`) — confirms the Batch-1 paper estimate exactly (no bridge-filtering delta). **>300 confirmed.** Reduction *proposal* (not implemented): (a) merge `jump`→`branch` → ~381; (b) drop zones with <5 trace steps → ~383; (a)+(b) combined → ~**327** (still >300). **Ruling (Opus, before Batch 3): KEEP the full 437 arms — no reduction in D2.C.** Rationale: (i) 437 × ~3 cold-start pulls ≈ 1.3k ≪ N=6000; (ii) even both levers only reach ~327 (still >300) — marginal benefit; (iii) `jump`-vs-`branch` and sparse zones carry analysis signal whose loss would degrade the V6-cTS-vs-V6-uniform comparison; (iv) the 300 figure is a "consider" advisory, within Pro's "100s" ceiling. Reduction levers stay documented and available if D2.E/D2.G measure a real cold-start cost — re-open then. |
| **ISS-3** | **`baseline_trace` (step→instruction) provenance for opcode-class derivation.** The Arguzz arm loop / `opcode_class_for_step` need a `step→instruction` map that `InspectionData` alone may not carry (v6_driver_v2 sources it from `host --trace`, separate from `A4_INSPECT`). If `build(arguzz_kinds=…)` is called with `baseline_trace=None`, arms would silently mis-bucket to `"system"`. | Batch 2 design (2026-06-20) | **RESOLVED** (Batch 2) | `SemanticArmUniverse.build` **raises `ValueError`** when `arguzz_kinds is not None and baseline_trace is None`. Layer-4 test asserts the guard; task 2.5 uses a real `host --trace` baseline. |
| **ISS-4** | **`MAPPING_INSTR_TO_OPCODE_CLASS` completeness.** `opcode_class_for_step` falls back to `"system"` for unknown instructions; if the real trace contains an RV32IM instruction not in the mapping, its arms are silently mis-bucketed. | Batch 2 design (2026-06-20) | **RESOLVED** (Batch 2) | Layer-3 bridge test asserts every `INSTR_KINDS` mnemonic maps to intended class (only `invalid` → `system`). Task 2.5 real-trace log: **0 unexpected fallback hits** on 3962 steps. |
| **ISS-5** | **Layer-3 bridge-test scope vs `_dispatch_arm` / `mutations.outcome` sequencing.** §4.8's Layer-3 row says `test_d2c_arguzz_bridge.py` asserts dispatch routing and outcome writes — but those are **Batch 3** (task 3.2). Batch 2 correctly scoped Layer-3 to bridge + real bandit only. | Batch 2 design (2026-06-20) | **RESOLVED** (Batch 3) | Batch 2: bridge test scoped without `A4Fuzzer`. **Batch 3:** fuzzer Arguzz path + driver write `record_mutation(..., outcome=…)`; ISS-6 dispatch via `ArmKey.parse(decision.arm_id)`. Layer-3 bridge test unchanged (no `A4Fuzzer` import). Optional Batch-4 augmentation: explicit dispatch assertions in bridge test file. |
| **ISS-6** | **Recovering the full 5-field `ArmKey` (esp. `surface`) from the cTS `BanditDecision` for dispatch routing.** `BanditDecision` (bandit_ts.py:31-42) carries `kind`, `zone`, `step`, `arm_id` (a string) — **not** the structured `surface` / `opcode_class` / `pre_post`. The fuzzer's `_run_v2_bandit_mutation` (fuzzer.py:979) currently unpacks only `kind, zone, step`, so it cannot tell an Arguzz arm from an A4 arm to route dispatch. | Batch 3 design (2026-06-20) | **RESOLVED** (Batch 3) | Batch 3: `_run_v2_bandit_mutation` recovers via **`ArmKey.parse(decision.arm_id)`** and branches on `arm.surface == ARGUZZ_EXEC_FAULT` → `_run_arguzz_cts_mutation`. **`BanditDecision` unchanged** — Tier-1 + Tier-2 golden traces green after landing. Central Planner verified cTS `select()` uses canonical `arm_id_for_decision` in all modes. |
| **ISS-7** | **`A4Fuzzer` `baseline_trace` capture for Arguzz strategies.** `A4Fuzzer` builds its universe from `A4_INSPECT` `InspectionData` (fuzzer.py:747, `SemanticArmUniverse.build(self.data, kinds)`) with **no step→instruction map**. For `selector_strategy in {v6_cTS, hybrid_cTS}` the build needs `baseline_trace` (ISS-3), which the fuzzer does not currently capture (only the driver does, via `host --trace`). | Batch 3 design (2026-06-20) | **RESOLVED** (Batch 3) | `_setup_v2_bandit` captures `baseline_trace` via `_capture_baseline_trace()` (`host --trace`) **only when** `arguzz_kinds is not None` (`v6_cTS`/`hybrid_cTS`). V5/A4 strategies keep `arguzz_kinds=None` — no trace cost; Tier-1 + Tier-2 golden traces green. |
| **ISS-8** | **CGC / Hook-3 empty on Arguzz `--inject` path (Batch 3 misdiagnosis).** Batch 3 reported 0 `compressed_global_coverage` / `global_failures` with only `A4_FAMILY_RESIDUE=1` and concluded Hook-3 was silent on inject. **Root cause (F13, Central Planner):** Hook-3 **capture** in `ffi.cpp` requires `A4_FAMILY_RESIDUE && (A4_MUTATION_CONFIG || A4_COVERAGE_TOUCH)`; emission needs SeqForward mode enabled by `A4_COVERAGE_TOUCH`. Missing co-trigger → zero residues → empty CGC. Not an analysis-scope limitation. | Batch 3 (Layer-5 smoke, 2026-06-20) | **RESOLVED** (Batch 4) | **Batch 4:** `DEFAULT_ARGUZZ_SUBPROCESS_ENV` in `arguzz_bridge.create_mutation_for_arm` sets both `A4_FAMILY_RESIDUE=1` and `A4_COVERAGE_TOUCH=1`. Local N=10: **CGC=16, global_failures=54** (seed 1243). **POS N=50** (flare, seed 1243): **CGC=88, global_failures=387**, 9/11 kinds, applied=47/skipped=3; `test_d2c_v6_uniform_pos_smoke.py` 6/6 green. |
| **ISS-9** | **Fuzzer Arguzz dispatch bypasses the bridge entry point (`create_mutation_for_arm`).** Batch 3 called `arguzz_invoke.run()` directly to pass env; bridge was production-dead. | Batch 3 review (Opus, 2026-06-20) | **RESOLVED** (Batch 4) | `create_mutation_for_arm` owns default Hook-3 env + optional `env=` merge. **Both** `fuzzer._run_arguzz_cts_mutation` and `v6_uniform_driver` route through the bridge. Bridge test asserts co-trigger env. Tier-1 + Tier-2 golden traces green. |

**POS-run policy (process rule, 2026-06-20):** *Any* test or script that runs **>10 real-binary mutations** at a time MUST be dispatched on **POS** (TUM testbed) via the canonical templates in `a4/docs/precloud/POS_PLAYBOOK.md` (`a4/pos/dispatch_audit.sh` + a manifest), **not** run locally inside `pytest`. Local real-binary gating tests are capped at **≤10 mutations** (enough to validate wiring/schema/`outcome` column). Larger statistical runs — kind-coverage (≥6/11), CGC presence, outcome distribution — are POS jobs whose output is pasted into the batch report. *(Batch 3's Layer-5 N=50 smoke ran locally at ~771 s with a 1800 s timeout hack — it predates this rule; it must be re-run on POS as part of Batch 4 / ISS-8.)*

*(Resolution log: … Batch 4 resolved ISS-8 (F13 config bug — missing `A4_COVERAGE_TOUCH` co-trigger; local N=10 + POS N=50 verified CGC+global_failures non-empty) and ISS-9 (bridge reconciliation). **F13 (Central Planner, 2026-06-20):** Batch-3 CGC-empty diagnosis was wrong — 1-line env bug. **F12:** N=10000 for cTS variants at D2.F. Remaining OPEN: ISS-1 D2.G fault-corroboration residual.)*

---

*End of D2.C spec v0.5 — LOCKED. Phase 0 complete (2026-06-19). §15 living-issue annex added 2026-06-20 (does not alter the locked design).*

**Next steps:** Batches 1–3 implemented & reviewed (2026-06-20); Batch 3 greenlit with follow-ups (ISS-8 POS CGC re-run, ISS-9 bridge reconciliation) folded into the Batch 4 kickoff (`composer/D2C_BATCH4_COMPOSER_KICKOFF.md`) — hybrid forerunner + 66-case registration + ISS-8/ISS-9 + closure.
...
5. §2 non-goal drift fix — the "other 7 V6 ENABLED_KINDS — Pro deprioritized; out of scope" bullet (pre-v0.4 wording) contradicted v0.4's V6-cTS = 11 decision; reworded to "Batch-1 real-binary gating for the 7 non-SELECTED kinds is deferred" (they remain in V6-cTS FULL scope).
6. §13 + §11-Batch-1 acceptance gate test baseline refreshed from `e2c2256`'s **609 passed / 19 skipped / 1 xfailed** to HEAD `dfd0ebe`'s **616 passed / 17 skipped / 0 xfailed** (ignore-replicates path), verified by re-run 2026-06-20. Full-suite-with-replicates = 632 passed / 17 skipped (649 collected). *(Composer surfaced the staleness during the Batch-1 kickoff review; numbers independently re-verified.)*
7. Spec status → **LOCKED v0.5**; Phase 0 closed.
8. **§15 living-issue annex added (post-lock, 2026-06-20)** — registry of foreseen/surfaced implementation risks (ISS-1 `include_trace`/`--trace` fault-tag policy; ISS-2 V6-cTS arm-space >300; ISS-3 `baseline_trace` provenance; ISS-4 `MAPPING_INSTR_TO_OPCODE_CLASS` completeness) with Status tracking. Does not alter the locked v0.5 design; updated as batches resolve issues.
