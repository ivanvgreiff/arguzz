...
| **Naming-overlap caveat** | Excluding the 4 A4-name-duplicate Arguzz kinds from Hybrid is **provisional budget control, NOT "names are redundant"** (Arguzz `PRE_EXEC_REG_MOD` ≠ A4 `PRE_EXEC_REG_MOD`). D2.G adds a rule to revisit if those kinds prove high-value in V6-cTS. **⚠️ REVISIT RULE — CONDITIONALLY armed (F26→F27, batch 1):** Hybrid's 4 selected kinds (`INSTR_WORD_MOD, PRE_EXEC_MEM_MOD, PRE_EXEC_PC_MOD, BR_NEG_COND`) **exclude `POST_EXEC_PC_MOD`** — the only *committed* kind that produces accepts (143 in V6-cTS, 341 in V6-uniform). **Caveat (F27, planner-conceded):** those accepts are expected to triage **mostly `cf_inert` no-op** (`+4` = committed next PC; no PC-provenance constraint). Add `POST_EXEC_PC_MOD` to Hybrid's set **only if** V6 triage yields a **non-empty divergent residue** of real candidates — not on the raw accept count. | §4 |
...
| **4** | **D2.G** territory analysis + propagation filter + Pro report | 🟡 SPEC v1.2 · B1–B3 DONE (batch 1, seeds 1234/1235) · classification logic **SETTLED** (F21–F27) · **B4 = critical path** | B1/B2 ✅ (triage live, 8/0/0 oracle) · B3 ✅ → **Provisional Case A on COVERAGE** (V6-cTS locs36/CGC617 > V6-uniform; Hybrid locs48.5) · **classification SETTLED:** F25 base-align gate landed (ISS-2), F27 `cf_inert` tier landed for `POST_EXEC_PC_MOD` (ISS-3). **B4 (F26 priority): regenerate tier-2 triage on ALL variants with current logic** — batch-1's only triage (132 Hybrid rows) was run with **stale pre-F22/F24 logic** ⇒ invalid; **all 1571 V6 accepts (1087 `INSTR_WORD_MOD` + 484 `POST_EXEC_PC_MOD`) are untriaged.** The 484 are expected **mostly `cf_inert` no-op** (`random_pc` +4 = committed next PC; INV1 30/30) — the soundness lead is only the **divergent residue** (if any survives triage), NOT the raw 484. Soundness Case UNDECIDED until V6 triaged (counts, not coverage). Full stats need **batch 2 (seed 1236) for n≥3** (currently p=nan at n=2). | [`IV_POS_8_D2_G_SPEC.md`](IV_POS_8_D2_G_SPEC.md) · [`composer/D2G_B4_COMPOSER_KICKOFF.md`](./composer/D2G_B4_COMPOSER_KICKOFF.md) |
