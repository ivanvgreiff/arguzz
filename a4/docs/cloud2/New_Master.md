...
**Status:** spec **LOCKED (v0.5)**; implementation not started — **awaiting Opus Batch 1 kickoff**.

### Phase 2 — Bernoulli floor for the new cTS variants — *§2 Phase 2, Q1*
...
**Spec:** `IV_POS_8_D2_BERNOULLI_FLOOR_SPEC.md` (**to write** — small; ~1–2 batches: scheduler change + unit tests + golden-trace confirming V5 path unchanged). *(Could alternatively fold into D2.D; kept separate per Pro's distinct-phase treatment.)*
...
- **Spec D2.D** `IV_POS_8_D2_D_SPEC.md` (**to write**) — variant CLI/`fuzzer.py` dispatch for `V5_control / V6_uniform / V6_cTS / Hybrid_cTS`; selects Bernoulli floor per-variant (V6-cTS/Hybrid only); **logs D1.C L1 channels in all fresh runs but keeps them INACTIVE** (Q3). **Ownership note:** the `applied_accounting_mode=True` *wiring* lands in **D2.C Batch 3** (task 3.2a in `fuzzer.py`); D2.D/D2.E only **validate/smoke-gate** it at the variant level (F9 — first real use). Batches: (1) selector-family + arm-universe wiring, (2) per-variant arm-coverage + applied-accounting smoke.
...
| **1** | **D2.C** Arguzz integration | 🔧 v0.5 LOCK · B1 pending kickoff | B1 primitive · B2 bridge+arm-count · B3 driver+fuzzer · B4 hybrid-smoke+closure | [`IV_POS_8_D2_C_SPEC.md`](./IV_POS_8_D2_C_SPEC.md) · `composer/D2C_BATCH*` (Opus kickoffs TBD) |
| **2** | **D2 Bernoulli floor** | ⛔ TO WRITE | B1 scheduler change + tests + V5-unchanged golden trace | `IV_POS_8_D2_BERNOULLI_FLOOR_SPEC.md` (TBD) |
| **3** | **D2.D** variant dispatch | ⛔ TO WRITE | B1 selector/arm-universe wiring · B2 arm-coverage + applied-accounting smoke + inactive-L1 logging | `IV_POS_8_D2_D_SPEC.md` (TBD) |
| **3** | **D2.E** integration tests | ⛔ TO WRITE | 5-layer test stack + normalize parity | `IV_POS_8_D2_E_SPEC.md` (TBD) |
| **3** | **D2.F** POS dispatch | ⛔ TO WRITE | F.1 smoke-gate (8) · F.2 production tail | master-plan §D2.F + `composer/D2F_*` kickoff (TBD) |
...
Legend: ✅ done · 🔧 in finalization · ⛔ spec to write · 💤 deferred/future.
