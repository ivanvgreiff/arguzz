#!/usr/bin/env python3
"""D2.B Batch 3 — TXN_CYCLE_PHASE_MOD attestation (empirical: dead arm on sha2-host)."""
...
from a4.core.trace_parser import parse_all_all_txns, parse_post_mut_dump, parse_txn_cycle_phase_mod
...
def _find_target(data: InspectionData) -> TxnCyclePhaseModTarget:
    for step in data.get_valid_steps_for_kind("TXN_CYCLE_PHASE_MOD"):
        candidates = get_targets_at_step(step, data)
        if candidates:
            return candidates[0]
    raise AssertionError("no TXN_CYCLE_PHASE_MOD target")


def _run_layers_2_3_4(target: TxnCyclePhaseModTarget) -> None:
    ...
    tag = parse_txn_cycle_phase_mod(mut_out)[-1]
    assert tag.new_cycle == new_cycle
    center = next(d for d in parse_post_mut_dump(mut_out) if d.txn_idx == target.txn_idx)
    assert center.cycle == new_cycle
    ...
    assert_trace_diff_matches_signature(
        diffs,
        {"primary": {"txn_idx": target.txn_idx, "field": "cycle", "old": target.original_cycle, "new": new_cycle}, "cascade": []},
    )

    broken_families = [fr["family"] for fr in (parse_family_residues(mut_out) or []) if fr.get("nonzero")]
    if broken_families or "verify segment" in mut_out or "constraint_fail" in mut_out.lower():
        pytest.fail(
    ...
    assert guard_fired
    pytest.xfail(
        "TXN_CYCLE_PHASE_MOD: unexpected dead arm on sha2-host (LSB XOR mutates trace; "
        "no rejection; verifier accepts). Contradicts spec §5.4 LIVE prediction."
    )


@pytest.mark.skipif(os.environ.get("A4_REAL_BINARY") != "1", reason="Set A4_REAL_BINARY=1")
class TestTxnCyclePhaseModAttestation:
