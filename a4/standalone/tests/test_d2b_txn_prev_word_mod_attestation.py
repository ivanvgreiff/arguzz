#!/usr/bin/env python3
"""D2.B Batch 1 — TXN_PREV_WORD_MOD attestation (Layers 2+3+4)."""
...
from a4.core.trace_parser import parse_all_all_txns, parse_post_mut_dump, parse_txn_prev_word_mod
...
def _find_target(data: InspectionData, strategy: str) -> TxnPrevWordModTarget:
    valid = data.get_valid_steps_for_kind("TXN_PREV_WORD_MOD")
    assert valid, "no valid TXN_PREV_WORD_MOD steps"
    for step in valid:
        candidates = get_targets_at_step(step, data, strategy=strategy)
        if candidates:
            return candidates[0]
    raise AssertionError(f"no {strategy} target on production trace")


def _run_layers_2_3_4(target: TxnPrevWordModTarget, strategy: str, seed: int = 42) -> None:
    rng = random.Random(seed)
    new_prev = generate_new_value(target, rng)

    with tempfile.TemporaryDirectory() as tmp:
        cfg_path = create_config(target, new_prev, Path(tmp) / "mut.json")

        pre_result = _run_host(
            {
                "A4_INSPECT": "1",
                "A4_DUMP_ALL_TXNS": "1",
            }
    ...
    evidence = parse_txn_prev_word_mod(mut_out)
    assert evidence, "Layer 2: missing <a4_txn_prev_word_mod> tag"
    tag = evidence[-1]
    assert tag.strategy == strategy
    assert tag.txn_idx == target.txn_idx
    assert tag.old_prev_word == target.original_prev_word
    assert tag.new_prev_word == new_prev
    ...
    assert center.prev_word == new_prev
    assert center.word == target.original_word
    ...
    assert tag.new_prev_word == center.prev_word, "Layer 4 cross-check failed"
    ...
    assert broken_families, (
        f"Hook 3: expected at least one nonzero family residue; got {family_residues}"
    )
    assert "memory" in broken_families, (
        f"Hook 3: expected memory family residue for prev_word mutation; "
        f"broken={broken_families}"
    )
...
class TestTxnPrevWordModAttestation:
    def test_layers_2_3_4_at_read(self):
        host = _host_binary()
        if not Path(host).is_file():
            pytest.skip(f"risc0-host not found at {host}")

        data = InspectionData.from_inspection(host, _host_args())
        target = _find_target(data, "at_read")
        _run_layers_2_3_4(target, "at_read")

    def test_layers_2_3_4_at_write(self):
        host = _host_binary()
        if not Path(host).is_file():
            pytest.skip(f"risc0-host not found at {host}")

        data = InspectionData.from_inspection(host, _host_args())
        target = _find_target(data, "at_write")
        _run_layers_2_3_4(target, "at_write")
